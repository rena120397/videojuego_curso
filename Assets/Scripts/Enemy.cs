...
public class Enemy : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        isGroundFloor = (Physics2D.Raycast(new Vector3(transform.position.x,
                            transform.position.y - floorCheckY,
                            transform.position.z),
            new Vector3(movHor, 0, 0), frontGrndRayDist, groundLayer));

        if (!isGroundFloor) //Mientras encuentre piso no haga nada hasta que  deje que  encontrar piso  y haga el cambio
            movHor = movHor * -1;

        //Choque con pared
        if (Physics2D.Raycast(transform.position,
            new Vector3(movHor, 0, 0), frontCheck, groundLayer))
            movHor = movHor * -1;
    ...
    private void getKilled() //función para cuando nuestro enemigo es eliminado
    {
